...
@section('content')
    <div class="mx-auto max-w-3xl px-4 py-8 sm:px-6 lg:px-8">
        <form method="POST" action="{{ route('service-center.profile.update') }}" class="rounded-lg border border-slate-200 bg-white p-6 shadow-sm">
            @csrf @method('PATCH')
            <h1 class="text-3xl font-black text-slate-950">Service center profile</h1>
            @include('riders.marketplace.partials.flash')
            <div class="mt-6 grid gap-4 sm:grid-cols-2">
                <label class="grid gap-2 text-sm font-bold">Center name<input name="center_name" value="{{ old('center_name', $profile->center_name) }}" class="rounded-md border-slate-300" required></label>
                ...
                <label class="grid gap-2 text-sm font-bold">Working hours<input name="working_hours" value="{{ old('working_hours', $profile->working_hours) }}" class="rounded-md border-slate-300"></label>
