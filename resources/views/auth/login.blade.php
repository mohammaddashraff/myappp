...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    <head>
        ...
        <title>Sign in | {{ config('app.name', 'Tayaran') }}</title>
            ...
            <section class="mx-auto grid min-h-[calc(100vh-4rem)] max-w-6xl items-center gap-8 lg:grid-cols-[0.95fr_1.05fr]">
                <aside class="max-w-xl">
                    <a href="{{ route('home') }}" class="inline-flex items-center gap-3">
                        <span class="flex size-12 items-center justify-center rounded-2xl bg-teal-400 text-xl font-black text-zinc-950 shadow-lg shadow-teal-950/40">T</span>
                        <span>
                            <span class="block text-2xl font-extrabold tracking-wide">Tayaran</span>
                    ...
                    <div class="mt-14">
                        ...
                        <p class="mt-5 max-w-lg text-base leading-7 text-zinc-300 sm:text-lg">
                            Use the account created during driver signup to view pending review status and continue with Tayaran once approved.
                        </p>
                    </div>
                </aside>
                ...
                <section class="rounded-3xl border border-white/10 bg-white p-5 text-zinc-950 shadow-2xl shadow-black/30 sm:p-8 lg:p-10">
                    <div class="border-b border-zinc-200 pb-6">
                        <p class="text-sm font-bold uppercase tracking-wide text-teal-700">Welcome back</p>
                        <h2 class="mt-1 text-3xl font-black text-zinc-950">Sign in</h2>
                    </div>
                    ...
                    <form method="POST" action="{{ route('login') }}" class="mt-8 space-y-5">
                        ...
                        <label for="remember_me" class="flex items-center gap-3 rounded-2xl border border-zinc-200 bg-zinc-50 px-4 py-3">
                            ...
                            <span class="text-sm font-semibold text-zinc-700">Keep me signed in</span>
                        </label>
                    ...
                    <div class="mt-7 rounded-2xl border border-zinc-200 bg-zinc-50 p-5">
                        <p class="text-sm font-bold text-zinc-700">New driver?</p>
