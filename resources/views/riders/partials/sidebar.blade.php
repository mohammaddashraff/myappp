...
@php($navDashboardRoute = $navDashboardRoute ?? 'rider.dashboard')
@php($navIsAdmin = $navIsAdmin ?? false)
@php($navShowsRiderGarage = $navDashboardRoute === 'rider.dashboard')
...
<aside class="w-full rounded-lg border border-slate-200 bg-white p-4 text-slate-950 shadow-sm lg:sticky lg:top-6 lg:self-start">
    ...
    <nav class="mt-3 space-y-1">
        <a href="{{ route($navDashboardRoute) }}" class="flex items-center gap-3 rounded-lg border px-3 py-2.5 text-sm font-bold transition {{ $active === 'dashboard' ? 'border-teal-200 bg-teal-50 text-teal-950' : 'border-transparent text-slate-600 hover:border-slate-200 hover:bg-slate-50 hover:text-slate-950' }}">
            <span class="size-2 rounded-full {{ $active === 'dashboard' ? 'bg-teal-500' : 'bg-slate-300' }}"></span>
            <span>{{ __('rider.nav_dashboard') }}</span>
        </a>
        ...
        @endif
        <a href="{{ route('rider.marketplace') }}" class="flex items-center gap-3 rounded-lg border px-3 py-2.5 text-sm font-bold transition {{ $active === 'marketplace' ? 'border-teal-200 bg-teal-50 text-teal-950' : 'border-transparent text-slate-600 hover:border-slate-200 hover:bg-slate-50 hover:text-slate-950' }}">
            <span class="size-2 rounded-full {{ $active === 'marketplace' ? 'bg-teal-500' : 'bg-slate-300' }}"></span>
    ...
    @if ($showAddButton && $active === 'dashboard' && ! $navIsAdmin)
        <div class="mt-6">
