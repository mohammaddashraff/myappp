...
<html lang="{{ app()->getLocale() }}" dir="{{ app()->getLocale() === 'ar' ? 'rtl' : 'ltr' }}">
    <head>
        ...
        <title>{{ __('rider.edit_motorcycle') }} | {{ __('rider.brand') }}</title>
                ...
                <div class="grid gap-6 py-2 lg:grid-cols-[0.72fr_1.28fr] lg:py-4">
                    <div class="flex flex-col justify-center">
                        ...
                        <h1 class="mt-2 text-3xl font-black leading-tight text-slate-950 sm:text-4xl">
                            {{ __('rider.edit_motorcycle_heading') }}
                        </h1>
                        <p class="mt-3 max-w-2xl text-sm leading-6 text-slate-600 sm:text-base">
                            {{ __('rider.edit_motorcycle_intro') }}
                        </p>
                    </div>

                    <form method="POST" action="{{ route('rider.motorcycles.update', $motorcycle) }}" enctype="multipart/form-data" class="rounded-lg border border-slate-200 bg-white p-6 text-slate-950 shadow-sm sm:p-8">
                        @csrf
                        @method('PATCH')
                        ...
                        @include('riders.motorcycles._form', [
                            'submitLabel' => __('rider.update_motorcycle'),
                        ])
                    </form>
                </div>
            </section>
